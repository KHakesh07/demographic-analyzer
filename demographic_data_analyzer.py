...
def calculate_demographic_data(print_data=True):
    ...
    percentage_bachelors = ((df['education'] == 'Bachelors').mean()*100).round(decimals=1)
    ...
    without_advanced_income = (df[~df['education'].isin(['Bachelors', 'Masters', 'Doctorate'])]['salary'].value_counts(normalize=True) * 100).loc['>50K']

    # with and without `Bachelors`, `Masters`, or `Doctorate`
    higher_ed_names = ['Bachelors', 'Masters', 'Doctorate']

    higher_education = df[df['education'].isin(higher_ed_names)]
    total_higher_Ed = higher_education.shape[0]

    # percentage with salary >50K
    higher_education_rich = round(higher_education[higher_education['salary'] == '>50K'].shape[0]/total_higher_Ed *100, 1)
    ...
    num_min_workers = (df[df['hours-per-week'] == min_work_hours]['salary'] == ">50k").mean()*100
...
# Displaying the results
    ...
    h = df[(df['native-country'] == 'India') & (df['salary'] == '>50K')]
    top_IN_occupation = h['occupation'].value_counts().idxmax()
